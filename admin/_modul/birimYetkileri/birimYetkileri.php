...
FROM 
	tb_personeller AS p
LEFT JOIN tb_birim_yetkileri AS biy ON biy.personel_id = p.id
													...
													if( $kategori['ust_id'] == $parent ){
														...
														if( $kategori['kategori'] == 0){
															$html .= "
																	<tr>
																		<td class=' bg-renk7' >
																			<input type='checkbox' class='' id='icheck_$kategori[id]' name='birim_idler[]' value='$kategori[id]' $secili  onclick='event.stopPropagation();'>
																			$kategori[adi]
																		</td>
																	</tr>";									
