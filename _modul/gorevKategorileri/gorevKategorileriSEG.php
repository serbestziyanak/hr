<?php
include "../../_cekirdek/fonksiyonlar.php";
$vt		= new VeriTabani();
$fn		= new Fonksiyonlar();

$islem				= array_key_exists( 'islem', $_REQUEST )			? $_REQUEST[ 'islem' ]			: 'ekle';
$gorev_kategori_id			= array_key_exists( 'gorev_kategori_id', $_REQUEST )	? $_REQUEST[ 'gorev_kategori_id' ]	: 0;
$alanlar			= array();
$degerler			= array();

$SQL_ekle			= "INSERT INTO tb_gorev_kategorileri SET ";
$SQL_guncelle 		= "UPDATE tb_gorev_kategorileri SET ";

$alanlar[]		= "universite_id";
$degerler[]		= $_SESSION['universite_id'];

foreach( $_REQUEST as $alan => $deger ) {
	if( $alan == 'islem' or  $alan == 'PHPSESSID' or  $alan == 'gorev_kategori_id') continue;

		$alanlar[]		= $alan;
		$degerler[]		= $deger;
}

$SQL_ekle		.= implode( ' = ?, ', $alanlar ) . ' = ?';

$SQL_guncelle 	.= implode( ' = ?, ', $alanlar ) . ' = ?';
$SQL_guncelle	.= " WHERE id = ?";

if( $islem == 'guncelle' ) $degerler[] = $gorev_kategori_id;


$SQL_tek_gorev_kategori_oku = <<< SQL
SELECT 
	*
FROM 
	tb_gorev_kategorileri 
WHERE 
	universite_id	= ? AND
	id 				= ?
SQL;


$SQL_sil = <<< SQL
DELETE FROM
	tb_gorev_kategorileri
WHERE
	id = ?
SQL;

$___islem_sonuc = array( 'hata' => false, 'mesaj' => 'İşlem başarı ile gerçekleşti', 'id' => 0 );

switch( $islem ) {
	case 'ekle':
		$sonuc = $vt->insert( $SQL_ekle, $degerler );
		if( $sonuc[ 0 ] ) $___islem_sonuc = array( 'hata' => $sonuc[ 0 ], 'mesaj' => 'Kayıt eklenirken bir hata oluştu ' . $sonuc[ 1 ] );
		else $___islem_sonuc = array( 'hata' => false, 'mesaj' => 'İşlem başarı ile gerçekleşti', 'id' => $sonuc[ 2 ] ); 
		$son_eklenen_id	= $sonuc[ 2 ]; 
		$gorev_kategori_id = $son_eklenen_id;
	break;
	case 'guncelle':
		//Güncellenecek olan tarife giriş yapılan firmaya mı ait oldugu kontrol ediliyor Eger firmaya ait ise Güncellenecektir.
		$tek_gorev_kategori_oku = $vt->select( $SQL_tek_gorev_kategori_oku, array( $_SESSION['universite_id'], $gorev_kategori_id ) ) [ 2 ];
		if (count( $tek_gorev_kategori_oku ) > 0) {
			$sonuc = $vt->update( $SQL_guncelle, $degerler );
			if( $sonuc[ 0 ] ) $___islem_sonuc = array( 'hata' => $sonuc[ 0 ], 'mesaj' => 'Kayıt güncellenirken bir hata oluştu ' . $sonuc[ 1 ] );
		}
	break;
	case 'sil':
		//Silinecek olan tarife giriş yapılan firmaya mı ait oldugu kontrol ediliyor Eger firmaya ait ise silinecektir.
		$tek_gorev_kategori_oku = $vt->select( $SQL_tek_gorev_kategori_oku, array( $_SESSION['universite_id'], $gorev_kategori_id ) ) [ 2 ];
		if (count( $tek_gorev_kategori_oku ) > 0) {
			$sonuc = $vt->delete( $SQL_sil, array( $gorev_kategori_id ) );
			if( $sonuc[ 0 ] ) $___islem_sonuc = array( 'hata' => $sonuc[ 0 ], 'mesaj' => 'Kayıt silinrken bir hata oluştu ' . $sonuc[ 1 ] );
		}
	break;
}
$_SESSION[ 'sonuclar' ] 		= $___islem_sonuc;
$_SESSION[ 'sonuclar' ][ 'id' ] = $gorev_kategori_id;
header( "Location:../../index.php?modul=gorevKategorileri&islem=guncelle&gorev_kategori_id=".$gorev_kategori_id );
?>